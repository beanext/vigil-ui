define(function (require) {
    ...
    app.controller('DemoCtrl', function ($scope, $http, $timeout, $interval) {
        ...
        $scope.refreshAddresses = function (address) {
            return $http.jsonp(
                'http://api.map.baidu.com/place/v2/suggestion?ak=RrQH3wFh5FAsgOgB3BZeCxE3&output=json&query=' + address + '&region=上海市&callback=JSON_CALLBACK'
            ).success(function (data, status, headers, config) {
                    var addresses = [];
                    angular.forEach(data.result, function (address, i) {
                        addresses[i] = {formatted_address: address.district + address.name};
                    })
